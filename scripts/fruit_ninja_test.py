...
# Define gift levels (example thresholds)
SMALL_GIFT_THRESHOLD = 20
MEDIUM_GIFT_THRESHOLD = 50
LARGE_GIFT_THRESHOLD = 90

# Define gift images or descriptions
SMALL_GIFT_IMAGE = "images/small_gift.png"
MEDIUM_GIFT_IMAGE = "images/medium_gift.png"
LARGE_GIFT_IMAGE = "images/big_gift.png"

# ... [Other existing code]

def get_gift_image(total_points):
    """ Determine the gift image based on the total_points. """
    if total_points < SMALL_GIFT_THRESHOLD:
        return None  # No gift for low total_points
    elif total_points < MEDIUM_GIFT_THRESHOLD:
        return SMALL_GIFT_IMAGE
    elif total_points < LARGE_GIFT_THRESHOLD:
        return MEDIUM_GIFT_IMAGE
    else:
        return LARGE_GIFT_IMAGE

def display_gift(screen, total_points):
    """ Display the appropriate gift on the screen. """
    gift_image_path = get_gift_image(total_points)
    if gift_image_path:
        gift_image = pygame.image.load(gift_image_path)
        gift_image = pygame.transform.scale(gift_image, (100, 100))  # Adjust size as needed
        screen.blit(gift_image, (GAME_WIDTH / 2 - 50, GAME_HEIGHT / 2 - 50))
...
HIGHEST_SCORE = 0
# check if highest score file exists, if not, create it
try:
...
def main():
    ...
    with MP_POSE.Pose(
        ...
        # 0 is the default camera, 1 is the second camera, etc.
        cap = cv2.VideoCapture(1)
        
        # if you want to use an IP camera, use this instead
        ...
        while cap.isOpened() and running:
            ...
            if type(start_fruit) is Fruit: 
                ...
                # display score and gift
                display_gift(screen, total_points)
                score_text = MAIN_NUMBER_FONT.render(f'Final Score: {total_points}', False, WHITE)
                screen.blit(score_text, (
                    round(GAME_WIDTH / 2) - score_text.get_width() / 2, 
                    round(GAME_HEIGHT / 2) - score_text.get_height() / 2))
                    # if score is higher than highest score, display new high score
                if total_points > HIGHEST_SCORE:
                    high_score_text = MAIN_NUMBER_FONT.render(f'New High Score!', False, WHITE)
                    screen.blit(high_score_text, (
                        round(GAME_WIDTH / 2) - high_score_text.get_width() / 2, 
                        round(GAME_HEIGHT / 2) + score_text.get_height() / 2))
                # if score > 50 player get a small gift, > 100 medium gift, > 150 large gift
                gift_image_path = get_gift_image(total_points)
                if gift_image_path:
                    gift_image = pygame.image.load(gift_image_path)
                    gift_image = pygame.transform.scale(gift_image, (100, 100))  # Adjust size as needed
                ...
                destroy = draw_start_end_fruit(
                    ...
                    right_circles, 
                    last_round)
                # If S button is pressed, start new game
                keys_pressed = pygame.key.get_pressed()
                if keys_pressed[pygame.K_s]:
                    destroy = True
                # If R button is pressed, refresh camera
                if keys_pressed[pygame.K_r]:
                    cap.release()  # Release the current camera
                    cap = cv2.VideoCapture(0)  # Re-open the camera
            ...
            display_menu(
                ...
                HIGHEST_SCORE if start_game else None)
            ...
            if game_ended:  # This condition should be based on your game logic
                # Calculate final score and display the gift
                final_score = total_points  # Use your game's score variable
                display_gift(screen, final_score)
                pygame.display.update()
...
def display_menu(fps, round_number=None, total_points=None, fruits_left=None, highest_score=None):
    ...
    if type(highest_score) is int:
        highest_score_text = MAIN_NUMBER_FONT.render(f'Highest Score: {highest_score}', False, WHITE)
        screen.blit(highest_score_text, (ratio(20), ratio(100)))
    ...
    title_text = TITLE_FONT.render(f'AI-FUDN', False, FPT_COLOR)
