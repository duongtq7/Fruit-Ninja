...
def main():
    ...
    with MP_POSE.Pose(
        ...
        # FFMPEG_CMD = f"ffmpeg -nostdin -flags low_delay -rtsp_transport tcp -i {stream_url} -pix_fmt bgr24 -an -vcodec rawvideo -f rawvideo -".split(" ")
        # WIDTH = 1920
        # HEIGHT = 1080
        ...
        while process.poll() is None:
            ...
            img = np.frombuffer(raw_frame, np.uint8) 
...
def display_menu(fps, round_number=None, total_points=None, fruits_left=None, highest_score=None):
    ...
    title_text = TITLE_FONT.render(f'FPT UNIVERISITY', False, FPT_COLOR)
